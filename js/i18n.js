...
const I18N = {
  zh: {
    ...
    'label.hlThr.title': '差異門檻：像素差 ≥ 門檻才標色',
    'btn.hlThrReset': '↺ 預設',
    ...
    'debug.mediaLine': '{side}：{name}｜{size}MB｜容器 {container}｜時長 {duration}｜解析度 {res}｜影片編碼 {codec}｜FPS {fps}｜聲音 {audio}',
    'debug.imageLine': '{side}：圖片 {name}｜{size}MB｜解析度 {res}｜類型 {type}',
    'debug.body': '<div><strong>媒體詳細資訊：</strong></div><div>{leftLine}</div><div>{rightLine}</div><div>同步偏移: {offset}s | 基準時長: {baseDur}</div><div>檢測FPS: {detectedFPS} | 目前設定: {fpsSetting}</div><div><strong>主時鐘引擎：</strong> {playState} | master={master}s | 影格 {frame}/{totalFrames}</div><div>左實際 currentTime: {leftCur} | 右實際: {rightCur}s</div><div><strong>瀏覽器支援：</strong> {formats}</div><div><strong>載入狀態：</strong> 左={leftReady}/4, 右={rightReady}/4</div>',
  },
  ...
  en: {
    ...
    'label.hlThr.title': 'Threshold: pixels with diff ≥ threshold get colored',
    ...
    'debug.mediaLine': '{side}: {name} | {size}MB | container {container} | duration {duration} | resolution {res} | video codec {codec} | FPS {fps} | audio {audio}',
    'debug.imageLine': '{side}: image {name} | {size}MB | resolution {res} | type {type}',
    'debug.body': '<div><strong>Media details:</strong></div><div>{leftLine}</div><div>{rightLine}</div><div>Sync offset: {offset}s | Base duration: {baseDur}</div><div>Detected FPS: {detectedFPS} | Current setting: {fpsSetting}</div><div><strong>Master clock engine:</strong> {playState} | master={master}s | frame {frame}/{totalFrames}</div><div>Left actual currentTime: {leftCur} | right actual: {rightCur}s</div><div><strong>Browser support:</strong> {formats}</div><div><strong>Load state:</strong> left={leftReady}/4, right={rightReady}/4</div>',
  },
};
